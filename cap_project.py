...
def load_to_database_creditcard(df, table_name):
...
def credit_card_dataset_ETL():
    creditcard_df, customer_df, branch_df = load_to_dataframe()
    processed_creditcard_df = mapping_process_creditcard_data(creditcard_df) #After mapping new_creditcard_df
    processed_customer_df = mapping_process_customer_data(customer_df) #After mapping new_customer_df
    processed_branch_df = mapping_process_branch_data(branch_df) #After mapping new_branch_df

    load_to_database_creditcard(processed_creditcard_df, 'CDW_SAPP_CREDIT_CARD')
    load_to_database_creditcard(processed_customer_df, 'CDW_SAPP_CUSTOMER')
    load_to_database_creditcard(processed_branch_df, 'CDW_SAPP_BRANCH')

#credit_card_dataset_ETL()
...
def fetch_transactions_by_zip_code_month_and_year():
    ...
    while True:
        ...
        if len(cust_zip) != 5 or not cust_zip.isnumeric():
            if cust_zip.lower() == 'exit':
...
def fetch_transactions_by_type():
    ...
    valid_transaction_types = ['EDUCATION', 'BILLS', 'HEALTHCARE', 'ENTERTAINMENT', 'GROCERY', 'GAS', 'TEST']
    while True:
        transaction_type = input("Please enter the Transaction type from the list: ['Bills', 'Healthcare', 'Test', 'Education', 'Entertainment', 'Gas', 'Grocery']: ")
        user_input = transaction_type.upper()
        if user_input not in valid_transaction_types:
            if transaction_type.lower() == 'exit':
                print("Returning to the main menu.")
                return
            else:
                print("\nInvalid input. Enter a valid transaction type or type 'exit' to go back to the main menu.\n")
        else:
            break
...
def fetch_transactions_by_state():
    ...
    valid_states = ['MN', 'IL', 'NY', 'FL', 'PA', 'NJ', 'CT', 'OH', 'MI', 'KY', 'MD','WA', 'CA', 'TX', 'NC', 'VA', 'GA', 'MT', 'AR', 'MS', 'WI', 'IN', 'SC', 'MA', 'IA', 'AL']

    while True:
        state = input("Please enter the state abbreviation from the below list.\n"
                    "'AL' - Alabama\n 'AR' - Arkansas\n 'CA' - California\n 'CT' - Connecticut\n "
                    "'FL' - Florida\n 'GA' - Georgia\n 'IL' - Illinois\n 'IN' - Indiana\n "
                    "'IA' - Iowa\n 'KY' - Kentucky\n 'MD' - Maryland\n 'MA' - Massachusetts\n "
                    "'MI' - Michigan\n 'MN' - Minnesota\n 'MS' - Mississippi\n 'MT' - Montana\n "
        ...
        if state not in valid_states:
            if state.lower() == 'exit':
                ...
                print("\nInvalid input. Enter a valid state abbreviation or type 'exit' to go back to the main menu.\n")
        else:
            break


    query = f""" 
            SELECT b.BRANCH_STATE, b.BRANCH_CODE, count(c.TRANSACTION_ID) as TOTAL_TRANSACTION, SUM(c.TRANSACTION_VALUE) as TOTAL_VALUE
    ...
    df = pd.DataFrame(data, columns=["BRANCH_STATE", "BRANCH_CODE", "TOTAL_TRANSACTIONS", "TOTAL_VALUE"])
    print(df)
    print("\n")
    cursor.close()
    connection.close()
...
def check_account_details():
    ...
    while True:
        ...
        if len(customer_ssn) != 9 or not customer_ssn.isnumeric():
            if customer_ssn.lower() == 'exit':
                print("Returning to the main menu.")
                return
            else:
    ...
    if account_details:
        for row in account_details:
            ...
            print(f"\n SSN: {SSN}\n FIRST_NAME: {FIRST_NAME}\n MIDDLE_NAME: {MIDDLE_NAME}\n LAST_NAME: {LAST_NAME}\n CREDIT_CARD_NUMBER: {CREDIT_CARD_NO}\n FULL_ADDRESS: {FULL_STREET_ADDRESS}\n CITY: {CUST_CITY}\n STATE: {CUST_STATE}\n COUNTRY: {CUST_COUNTRY}\n PHONE: {CUST_PHONE}\n ZIP: {CUST_ZIP}\n EMAIL: {CUST_EMAIL}\n LAST UPDATED: {LAST_UPDATED}")
            print("\n")
    else:
        print("\nNo record available.\n")
...
def modify_account_details():
    while True:
        ...
        print("\nWhat do you want to update? Please select a number from below list:")
...
def monthly_bill():
    ...
    while True:    
        ...
        if len(month) == 1:
            month = '0' + month
        if not month.isnumeric() or not (int(month) >= 1 and int(month) <= 12):
            if month.lower() == 'exit':
                print("Returning to the main menu.")
                return
            else:
...
def Transaction_range():
    ...
    while True:
        ...
        if len(start_date) != 8 or not start_date.isnumeric():
    ...
    while True:
        ...
        if len(end_date) != 8 or not end_date.isnumeric():
...
def application_front_end():
    while True:
        print("\n \n -----------------------------Welcome to Application Front-End program-------------------------------")
        print("""
        Please select a module to dive deeper into its functional requirements:
...
def plot_transaction_type_with_highest_count():
    ...
    #plt.cm.tab10 -> This referes to the tab10 colormap provided by matplotlib.(range of color)
    #(np.linspace(0, 1, len(df))) -> function from the numpy library( array of evenly spaced numbers over a specified range)
    ...
    for idx, bar in enumerate(bars):
        ...
        #Text is vertically centered on the bar(va=center) and is right-aligned to the specified x-coord(ha=right)
...
def plot_state_with_high_customer_count():
    ...
    for bar in bars:
        ...
        #bar.get_x() -> fetches the x-coordinate of the left side of the bar
        #bar.get_width()/2-> width of the bar
        #adding half of hte width to the x-coordinate position the text in the mid of the bar
        #yval->y-co where text wil placed
    plt.xlabel("State")
...
def plot_top_three_months_largest_vol_tran_count():
    ...
    #To Identify the top 3 months based on transaction counts from Pandas library(nlargest)
    #.tolist() -> converts pandas series into a regular python list
    ...
    #iterrows()-> method of pd 'df'. It allows to iterate over each row of the df as s series. i is index and row will contain data
...
def main():
    while True:
        ...
        3. Loan Application Dataset - Data Analysis and Visualization Module
        4. ETL - Credit Card dataset
        5. ETL - Loan Application dataset
        6. Exit
        """)
        choice = input("Enter your choice: ")
        if choice == '1':
